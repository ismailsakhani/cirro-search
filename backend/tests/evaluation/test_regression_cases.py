...
CASES_PATH = Path(__file__).resolve().parents[3] / "evaluation" / "cases" / "regression.yaml"
TEST_INDEX = f"cirro-search-regression-{uuid.uuid4().hex[:8]}"
...
def test_regression_case(pipeline: SearchPipeline, case: dict) -> None:
    ...
    assert response.results, (
        f"No results for query '{case['query']}' (mode={mode.value}){note_str}"
    )

    top = response.results[0]
    assert _result_matches(top, case["expect"]), (
        f"Query '{case['query']}' (mode={mode.value}) expected {case['expect']} but got "
        f"{{'type': '{top.type.value}', 'iata': '{top.iata}', "
        f"'iata_flight': '{top.iata_flight}', 'gate': '{top.gate}', "
        f"'airport_code': '{top.airport_code}'}}{note_str}"
    )
